...
export default component$(() => {
	...
	const handleSubmit:any= $(async (values:any) => {
		const rawdata = new FormData(values.target);

		const {success } = await formSchema.spa([...rawdata.entries()]);
		if(success) attestation(new AbortController(), "XXX")
	}
	)
	...
	return (
		<>
			<h2>Sign in</h2>
			<Form onSubmit$={handleSubmit} action={loginAction}>
